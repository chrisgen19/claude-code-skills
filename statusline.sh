...
# Line 1: Model | Project dir | Git branch + status
...
gh_prefix=""
if [ -n "$gh_user" ]; then
  gh_prefix="${DIM}@${gh_user}${RESET}  "
fi

# -- LINE 1: GH User | Model | Dir | Git --
printf '%b\n' "${gh_prefix}${BOLD}[${model}]${RESET}${ver_info}  ${short_dir}${git_info}"
